...
class PEZDispenserGUI:
    ...
    def draw_dispenser(self):
        ...
        # Calculate total height of tablets
        total_tablets_height = len(self.stack) * self.tablet_height
        spring_height = self.dispenser_height - total_tablets_height  # Spring occupies remaining height

        # Ensure the spring starts from the bottom and compresses upward
        spring_top = y1 + spring_height

        # Draw spring below the tablets
        wave_height = 10
        ...
        # Draw tablets starting from the bottom, just above the spring
        for i, tablet in enumerate(self.stack.items):
            y = spring_top - (i + 1) * self.tablet_height  # Stack the tablets upwards, starting from the spring

...
class PEZDispenserGUI:
    ...
    def draw_dispenser(self):
        ...
        spring_top = y1 + tablets_height  # Adjust spring position based on how many tablets are present
        spring_bottom = y1 + self.dispenser_height  # Spring should end at the bottom
        
        # Draw tablets (oval shape) from the top of the dispenser
        for i, tablet in enumerate(self.stack.items):
            y = y1 + i * self.tablet_height  # Stack tablets from the top going downward
        ...
        # Draw spring below the tablets, compressing as tablets are added
        wave_height = 10

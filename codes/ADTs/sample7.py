...
class PEZDispenserGUI:
    ...
    def draw_dispenser(self):
        ...
        # Draw spring with reduced padding for wider spring
        padding = 20  # Reduced from 30 to 20 for wider spring
        spring_width = self.dispenser_width - (2 * padding)
        wave_height = 8  # Keeping the same wave height
        num_waves = max(15, int(spring_height / (wave_height * 2)))
        
        # Draw spring waves as a continuous line
        wave_points = []
        for i in range(num_waves * 2 + 1):
            x = x1 + padding + (spring_width * (i % 2))
